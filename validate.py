...
def valid_create_inputs(inputs):
    ...
    if not valid_name(inputs[4]):
        print_error("The job title is invalid, please input an alpha name for job title.")
        return False
...
def valid_delete_inputs(inputs):
    if len(inputs) != 1:
        print_error("Not a valid number of inputs, if you wish to delete an employee, please enter 4 then type the employee id.")
        return False
    if not valid_number(inputs[0]):
        print_error("The employee id should be a digital number, please input a correct employee to delete.")
        return False
    return True
