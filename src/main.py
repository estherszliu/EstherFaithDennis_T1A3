...
def create_employee(filename, employee_data):
    ...
    for employee in employees:
        if employee.employee_id == id:
            print_error(
                "The employee id already exist, please input a valid employee id.")
            return False
    ...
    for employee in employees:
        ...
        for i in range(0, len(search_data), 2):
            ...
            if field == '--min-salary' and employee.salary < int(value):
                is_match = False
                break
            if field == '--max-start-date' and datetime.strptime(
                    employee.start_date, '%d/%m/%y') > datetime.strptime(value, '%d/%m/%y'):
                is_match = False
                break
            if field == '--min-start-date' and datetime.strptime(
                    employee.start_date, '%d/%m/%y') < datetime.strptime(value, '%d/%m/%y'):
                is_match = False
                break
        if is_match:
...
def update_data(filename, update_employee):
    ...
    if employee is None:
        print_error(
            "Update employee can not found, please input the correct employee id.")
        return False
...
def delete_data(filename, delete_employee):
    ...
    if employee is None:
        print_error(
            "Delete employee can not found, please input the existing employee id to delete.")
        return False
